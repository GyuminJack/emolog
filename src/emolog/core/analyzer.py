...
class EmotionAnalyzer:
    ...
    def show_stats(self, period: str = "week"):
        ...
        # Average intensity
        intensities = [entry.get('intensity', 5) for entry in entries if entry.get('intensity')]
        if intensities:
    ...
    def show_patterns(self):
        ...
        if len(entries) < 5:
            console.print("[yellow]패턴 분석을 위해서는 최소 5개 이상의 기록이 필요합니다.[/yellow]")
            return
    ...
    def show_triggers(self):
        ...
        negative_emotions = ['스트레스', '불안', '좌절', '화남', '긴장', '피로', 
                            '걱정', '슬픔', '짜증', '실망', '우울', '두려움']
        
        # Filter negative emotion entries
        negative_entries = [
            entry for entry in entries 
            if entry.get('emotion', '').lower() in [e.lower() for e in negative_emotions]
        ]
        ...
        situations = [entry['situation'] for entry in negative_entries if entry.get('situation')]
    ...
    def show_timeline(self, period: str = "today"):
        ...
        for entry in entries:
            ...
            console.print(f"{time_str}  {emotion}({intensity}) [{bar_color}]{bar}[/{bar_color}]  {situation}")
    ...
    def _analyze_day_patterns(self, entries: List[Dict[str, Any]]) -> List[str]:
        ...
        for day, emotions in day_emotions.items():
            if len(emotions) >= 3:  # Need sufficient data
                ...
                negative_count = sum(1 for emotion, _ in emotions if emotion in negative_emotions)
                
                if negative_count / len(emotions) > 0.6:  # 60% negative
                    patterns.append(f"{day}에 부정적인 감정이 집중되는 경향 ({negative_count}/{len(emotions)})")
        ...
        for time_slot, emotions in time_emotions.items():
            if len(emotions) >= 3:
                ...
                if avg_intensity > 6.5:
                    patterns.append(f"{time_slot} 시간대에 감정 강도가 높은 편 (평균 {avg_intensity:.1f})")
    ...
    def _analyze_tag_patterns(self, entries: List[Dict[str, Any]]) -> List[str]:
        ...
        for tag, emotions in tag_emotions.items():
            if len(emotions) >= 3:
                avg_intensity = sum(intensity for _, intensity in emotions) / len(emotions)
                if avg_intensity > 7:
                    patterns.append(f"'{tag}' 태그가 있을 때 감정 강도가 높음 (평균 {avg_intensity:.1f})")
        ...
        for context, emotions in context_emotions.items():
            if len(emotions) >= 5:
                ...
                if negative_count / len(emotions) > 0.7:
                    patterns.append(f"'{context}' 컨텍스트에서 부정적 감정이 빈번함 ({negative_count}/{len(emotions)})")
        
        return patterns
    
    def _analyze_triggers(self, situations: List[str], entries: List[Dict[str, Any]]) -> List[Tuple[str, int, float]]:
    ...
    def _analyze_tag_triggers(self, entries: List[Dict[str, Any]]) -> List[Tuple[str, int, float]]:
    ...
    def _get_emotion_color(self, emotion: str) -> str:
        ...
        positive_emotions = ['기쁨', '만족', '자신감', '안도', '설렘', '평온', '행복', '감사']
        negative_emotions = ['스트레스', '불안', '좌절', '화남', '긴장', '피로', '걱정', '슬픔']
