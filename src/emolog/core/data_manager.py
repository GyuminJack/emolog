...
class DataManager:
    ...
    def _parse_timestamp_to_kst(self, timestamp_str: str) -> datetime:
        ...
        if timestamp_str.endswith('Z'):
            return datetime.fromisoformat(timestamp_str.replace('Z', '+00:00')).astimezone(KST)
    ...
    def save_entry(self, entry: Dict[str, Any]) -> str:
        ...
        if 'timestamp' in entry and entry['timestamp']:
            ...
            if timestamp_str.endswith('Z'):
                timestamp = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00')).astimezone(KST)
        ...
        file_path = self.entries_dir / str(year) / f"{month:02d}" / f"{year}{month:02d}{day:02d}.jsonl"
            ...
            if month_dir.exists():
                for jsonl_file in month_dir.glob("*.jsonl"):
                    try:
                        with open(jsonl_file, 'r', encoding='utf-8') as f:
                            for line in f:
                                if line.strip():
                                    ...
                                    timestamp_str = entry['timestamp']
                                    if timestamp_str.endswith('Z'):
                                        entry_time = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00')).astimezone(KST)
                                    elif '+' in timestamp_str:
                                        entry_time = datetime.fromisoformat(timestamp_str).astimezone(KST)
                                    else:
                                        # Assume it's already in KST
                        ...
                        console.print(f"[yellow]Warning: Could not parse entry in {jsonl_file}: {e}[/yellow]")
    ...
    def export_data(self, format_type: str, period: str = 'all', output_path: Optional[str] = None):
        ...
        if output_path is None:
            ...
            output_path = self.exports_dir / f"emolog_{period}_{timestamp}.{format_type}"
        else:
    ...
    def _export_csv(self, entries: List[Dict[str, Any]], output_path: Path):
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            fieldnames = ['timestamp', 'situation', 'emotion', 'intensity', 
                         'body_reaction', 'thought', 'context', 'tags']
    ...
    def reset_data(self, period: str = 'all', skip_confirm: bool = False):
        ...
        if not skip_confirm:
            if period == 'all':
                console.print("[red]이 작업은 되돌릴 수 없습니다. 백업을 먼저 생성하는 것을 권장합니다.[/red]")
                create_backup = Confirm.ask("삭제 전에 백업을 생성하시겠습니까?", default=True)
                if create_backup:
                    self.create_backup()
            
            confirmed = Confirm.ask(f"정말로 {description}를 삭제하시겠습니까?", default=False)
            if not confirmed:
        ...
        console.print(f"[green]✅ {description} {len(entries_to_delete)}개가 삭제되었습니다.[/green]")
    ...
    def selective_delete(self, period: str = 'week'):
        ...
        for i, entry in enumerate(entries, 1):
            ...
            situation = entry.get('situation', '')[:30] + ('...' if len(entry.get('situation', '')) > 30 else '')
        ...
        console.print("[dim]예: 1,3,5 또는 'all' (모두 선택) 또는 'cancel' (취소)[/dim]")
        ...
        console.print(f"[green]✅ {len(selected_entries)}개 항목이 삭제되었습니다.[/green]")
    ...
    def edit_entry(self, period: str = 'week'):
        ...
        field_choice = Prompt.ask("선택", choices=["1", "2", "3", "4", "5", "6", "7", "8", "9"], default="9")
        ...
        if field_choice == "1" or field_choice == "8":
            updated_entry['situation'] = self._edit_situation(selected_entry.get('situation', ''))
        
        if field_choice == "2" or field_choice == "8":
            updated_entry['emotion'] = self._edit_emotion(selected_entry.get('emotion', ''))
        
        if field_choice == "3" or field_choice == "8":
            updated_entry['intensity'] = self._edit_intensity(selected_entry.get('intensity', 5))
        
        if field_choice == "4" or field_choice == "8":
            updated_entry['body_reaction'] = self._edit_body_reaction(selected_entry.get('body_reaction', ''))
        
        if field_choice == "5" or field_choice == "8":
            updated_entry['thought'] = self._edit_thought(selected_entry.get('thought', ''))
        
        if field_choice == "6" or field_choice == "8":
            updated_entry['context'] = self._edit_context(selected_entry.get('context', ''))
    ...
    def _display_entry_details(self, entry: Dict[str, Any]):
        ...
        content.append(f"😊 감정: {entry.get('emotion', '')} (강도: {entry.get('intensity', '')}/10)")
    ...
    def _edit_tags(self, current: List[str]) -> List[str]:
        ...
        new_value = Prompt.ask("새로운 태그 (쉼표로 구분)", default=current_tags_str).strip()
        if new_value:
            tags = [tag.strip() for tag in new_value.split(',') if tag.strip()]
            return tags[:5]  # Limit to 5 tags
        return []
    
    def _update_entry(self, original_entry: Dict[str, Any], updated_entry: Dict[str, Any]):
