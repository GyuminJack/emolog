...
class EmotionLogger:
    ...
    def start_interactive_logging(self):
        ...
        if not context:
            console.print("[yellow]컨텍스트를 선택하지 않으면 기록할 수 없습니다.[/yellow]")
            return
        ...
        if self._confirm_entry(entry):
            ...
            console.print(f"[green]✅ 기록이 저장되었습니다! (ID: {entry_id[:8]})[/green]")
        else:
    ...
    def _get_situation(self) -> str:
        ...
        console.print("[dim]예: '회의에서 디자인 변경 요구 받음', '코드리뷰 받음'[/dim]")
    ...
    def _get_emotion(self) -> str:
        ...
        max_rows = max(len(self.emotions["positive"]), 
                      len(self.emotions["negative"]), 
                      len(self.emotions["neutral"]))
        
        for i in range(max_rows):
            positive = self.emotions["positive"][i] if i < len(self.emotions["positive"]) else ""
            negative = self.emotions["negative"][i] if i < len(self.emotions["negative"]) else ""
            neutral = self.emotions["neutral"][i] if i < len(self.emotions["neutral"]) else ""
    ...
    def _get_body_reaction(self) -> str:
        ...
        console.print("[dim]위 목록에서 선택하거나 직접 입력하세요 (Enter로 건너뛰기)[/dim]")
    ...
    def _get_thought(self) -> str:
        ...
        console.print("[dim]예: '망했다', '어떻게 해결하지', '잘될거야' (Enter로 건너뛰기)[/dim]")
    ...
    def _get_context(self) -> str:
        ...
        for i, ctx in enumerate(self.contexts, 1):
            console.print(f"[dim]{i}. {ctx['emoji']} {ctx['label']} ({ctx['key']})[/dim]")
        ...
        while True:
            ...
            console.print("[red]올바른 번호를 선택하거나 컨텍스트를 입력해주세요.[/red]")
    ...
    def _confirm_entry(self, entry: Dict[str, Any]) -> bool:
        ...
        panel_content.append(f"😊 감정: {entry['emotion']} (강도: {entry['intensity']}/10)")
